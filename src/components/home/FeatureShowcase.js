      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
          <div className="relative">
            {features.map((feature, index) => (<div key={index} className={`transition-all duration-500 ${activeFeature === index
                ? 'opacity-100 transform translate-y-0'
                : 'opacity-0 absolute inset-0 transform translate-y-4'}`} style={{ display: activeFeature === index ? 'block' : 'none' }}>
                <img src={feature.image} alt={feature.title} className="rounded-2xl shadow-2xl"/>
              </div>))}
          </div>

          <div className="mt-10 lg:mt-0 space-y-6">
            {features.map((feature, index) => (<div key={index} className={`p-6 rounded-xl cursor-pointer transition-all duration-300 ${activeFeature === index
                ? 'bg-blue-50 shadow-md'
                : 'hover:bg-gray-50'}`} onClick={() => setActiveFeature(index)}>
                <div className="flex items-center">
                  <feature.icon className={`h-8 w-8 ${activeFeature === index ? 'text-blue-600' : 'text-gray-400'}`}/>
            ...
            <div className="pl-12 pt-4">
              <Button variant="primary" icon={ArrowRight} iconPosition="right" className="hover-scale">
