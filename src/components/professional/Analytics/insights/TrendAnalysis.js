...
export function TrendAnalysis({ data }) {
    return (<div className="space-y-4">
      {data.map((item) => {
            ...
            return (<div key={item.metric} className="bg-white rounded-lg p-4">
            ...
            <div className="mt-2">
              <div className="h-2 bg-gray-200 rounded-full">
                <div className={`h-2 rounded-full ${isPositive ? 'bg-green-500' : 'bg-red-500'}`} style={{ width: `${(item.current / item.target) * 100}%` }}/>
              </div>
            </div>
          </div>);
        })}
