...
export function TrendAnalysis({ data }) {
    return (_jsx("div", { className: "space-y-4", children: data.map((item) => {
            ...
            return (_jsx("div", { className: "bg-white rounded-lg p-4", children: _jsxs("div", { className: "flex justify-between items-center", children: [_jsx("span", { className: "text-sm font-medium text-gray-500", children: item.metric }), _jsxs("div", { className: `flex items-center ${isPositive ? 'text-green-600' : 'text-red-600'}`, children: [isPositive ? _jsx(TrendingUp, { className: "h-4 w-4 mr-1" }) : _jsx(TrendingDown, { className: "h-4 w-4 mr-1" }), formatPercentage(change)] }), _jsx("div", { className: "mt-2", children: _jsx("div", { className: `h-2 rounded-full ${isPositive ? 'bg-green-500' : 'bg-red-500'} trend-bar`, "data-width": item.current / item.target * 100 }) })] }) }, item.metric));
        }) }));
}
