...
export function generateInsights(metrics) {
    ...
    return insights.sort((a, b) => b.priority === 'high' ? 1 : a.priority === 'high' ? -1 : 0);
}
export function identifyAnomalies(data, threshold = 2) {
    const mean = data.reduce((a, b) => a + b, 0) / data.length;
    const stdDev = Math.sqrt(data.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / data.length);
    return data.map((value, index) => Math.abs(value - mean) > threshold * stdDev ? index : -1).filter(index => index !== -1);
}
